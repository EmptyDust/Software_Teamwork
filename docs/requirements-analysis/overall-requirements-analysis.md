...
| `智能问答需求说明书(1).md` | 多轮对话、SSE 流式回答、Agent/ReAct 循环、MCP 工具调用、引用溯源、问答配置、问答统计 |
...
| 智能问答 | `qa` | `knowledge`、`auth`、`ai-gateway`、MCP Client、可选 `file` |
...
| 文档解析 | 把原始文件转换为文本化或结构化内容，为切片和引用做准备 | MinIO 原始文件；内部 Parser HTTP 服务；PaddleOCR 后端；解析配置；文档类型 | 解析产物引用；解析状态；失败原因；可选解析文本预览 | P0 |
...
| 知识管理配置 | 管理嵌入模型、重排序模型、解析器并发和后端选择 | 管理员权限；AI Gateway profileId、模型名、维度；解析器配置 | 配置读取/更新 API；脱敏配置；配置变更时间和操作人 | P1 |
...
| Agent/ReAct 循环 | 让模型在受控范围内选择工具、观察结果并继续生成，替代固定后端流程编排 | 会话上下文；AI Gateway Function Calling；工具白名单；用户权限；最大迭代和超时配置 | Agent Run 状态；模型调用摘要；安全处理步骤 | P0 |
| MCP 工具调用编排 | 通过 MCP Client 发现和执行 `search_knowledge`、`get_citation_source` 等工具 | MCP tools/list；tools/call；工具 JSON Schema；权限校验；幂等键 | 工具调用摘要；工具结果摘要；错误码；SSE tool 事件 | P0 |
| 知识检索工具 | 将用户问题转为知识检索工具调用，组织上下文给 LLM | `knowledge` 检索 API 或 Knowledge MCP Server；默认知识库配置；Top K、阈值、标签过滤 | 对问答生成提供检索上下文；对前端返回检索摘要和引用候选 | P0 |
...
| 报告配置 | 管理 LLM、默认模板、导出格式和编号模式 | 管理员权限；AI Gateway profileId、模型名；模板 ID；默认导出配置；默认 style profile | 报告配置读取/更新 API；脱敏模型配置 | P1 |
...
- 智能问答契约覆盖会话、消息、Agent Run、MCP 工具调用摘要、流式问答、引用、检索测试和问答配置。
...
| Q4 | OCR 和文档解析首期使用内部 Parser HTTP 服务并预留 PaddleOCR 后端；通过 `parser.baseUrl`、`apiKey`、`timeoutSeconds`、`maxConcurrency` 配置。 | 影响文件处理链路和部署依赖 |
